...
from test.tracker_implement.basetracker import BaseTracker


class MultiTempTrack(BaseTracker):
    def __init__(self, params):
        super(MultiTempTrack, self).__init__(params)
        network = build_multitemptrack(params.cfg, training=False)
        ...
        self.state = None
        self.temp_token = network.temp_token
    ...
    def track(self,drone_id, image1, image2, image3, info1: dict = None, info2: dict = None, info3: dict = None):
        ...
        ######################测试阶段可视化
    #     save_dir = "/data/yakun/MultiTrack/output/debug/test_vis"
...
# 通过可视化发现，redetect预测出来的框可能产生很大的偏差。后面会纠正嘛？    
        ...
        if tmp_s_factor > 8:
            ...
            if isinstance(out_dict_ctr, list):
                out_dict_ctr = out_dict_ctr[-1]
