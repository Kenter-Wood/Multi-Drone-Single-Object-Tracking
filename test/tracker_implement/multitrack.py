...
class MultiTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(MultiTrack, self).__init__(params)
        network = build_track_three(params.cfg, training=False)
        print("get there")
    ...
    def track(self,drone_id, image1, image2, image3, info1: dict = None, info2: dict = None, info3: dict = None):
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(                  # 在forward这里要把三个模板传入模型
                template1=self.z_dict1.tensors, template2=self.z_dict2.tensors,template3=self.z_dict3.tensors, search=x_dict.tensors, ce_template_mask=self.box_mask_z)
        # add hann windows
    ...
    def three_search_redetect(self, image_a, image_b, drone_id, state_b, tmp_factor = 4.0, tmp_s_factor = 12.0, info_a: dict = None, info_b: dict = None):       
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(                  # 在forward这里三个模板都是B的
                template1=search_b.tensors, template2=search_b.tensors, template3=search_b.tensors, search=search_a.tensors, ce_template_mask=self.box_mask_z)
...
# 通过可视化发现，redetect预测出来的框可能产生很大的偏差。后面会纠正嘛？    
        ...
        if tmp_s_factor > 8:
            ...
            with torch.no_grad():
                out_dict_ctr = self.network.forward(                 
                    template1=search_b.tensors, template2=search_b.tensors, template3=search_b.tensors, search=search_ctr.tensors, ce_template_mask=self.box_mask_z)   # 在中间用搜索区域检测
    ...
    def feature_matching_redetect(self, image_a, image_b, drone_id, state_b, tmp_factor=4.0, tmp_s_factor=12.0, info_a: dict = None, info_b: dict = None):
        ...
        # 转化成灰度图。ref是作为参考的模板，find是搜寻范围，这里定为整张图（暂时）
        ...
        # cv2.rectangle(image_with_bbox, (x1, y1), (x1 + w, y1 + h), color=(255, 255, 0), thickness=2)  # 绘制绿色边界框

        # match_img = cv2.drawMatches(x_patch_arr_b, kp_ref, x_patch_arr, kp_find, 
        #                             good_matches, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
        
        # debug_dir = "/data/yakun/Mytrack/debug/redetect"
        ...
        # debug_dir = "/data/yakun/Mytrack/debug/matching"
        # os.makedirs(debug_dir, exist_ok=True)
