...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            base_results_path = '{}/threemdottest/{}'.format(trk.results_dir, seq.name)
...
def three_fuse_extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset_A)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            base_results_path_a = '{}/threemdottest/{}'.format(trk.results_dir, seq.name)
            base_results_path_b = '{}/threemdottest/{}'.format(trk.results_dir, dataset_B[seq_id].name)
            base_results_path_c = '{}/threemdottest/{}'.format(trk.results_dir, dataset_C[seq_id].name)
