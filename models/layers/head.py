...
# conv是一个标准Conv-BN-ReLU层
# plane是通道数
def conv(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1,
         freeze_bn=False):
    # 按照是否采用freezebn方法分成两种，但都是Conv-BN-ReLU层
    # dilation是采样间隔，若为2则与原图相乘时会隔一位
    if freeze_bn:
...
# 前传得到地图和边界框
    def forward(self, x, gt_score_map=None):
        """ Forward pass with input x. """
        # 获取三个热力图，然后把地图和对应的检测框返回
        score_map_ctr, size_map, offset_map = self.get_score_map(x)

        # assert gt_score_map is None
        # 基于gt给出的热力图，或计算到的中心点热力图，获取bbox。
...
# 从热力图计算bbox
    def cal_bbox(self, score_map_ctr, size_map, offset_map, return_score=False):
        # 将关于中心点的分数图展平，计算分数最高的点，它的索引认为是中心点，索引之后重新变成二维坐标
        max_score, idx = torch.max(score_map_ctr.flatten(1), dim=1, keepdim=True)
        idx_y = idx // self.feat_sz
        idx_x = idx % self.feat_sz
        # 把一维索引扩展成二维，从而从尺寸和偏置图读取相应的信息
        idx = idx.unsqueeze(1).expand(idx.shape[0], 2, 1)
        # 尺寸和偏置图与中心图分辨率相同，目的是计算每个点的框的尺寸和中心偏移量
        ...
        # cx, cy, w, h
        # 通过粗略的中心位置和偏置获得边界框，feat_sz是热力图的分辨率。这样做把中心归化到[0,1]范围内。
        # 后面拼接上w和h。
...
# 生成中心，偏置，尺寸图
    def get_score_map(self, x):
        # 通过sigmoid把x内所有值归一化，并用min和max夹断，给y。
        def _sigmoid(x):
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, BN=False):
        ...
        # h是一个重复的列表，一共有num_layers-1个，全部是hidden_dim
        h = [hidden_dim] * (num_layers - 1)
        # 列表加代表拼接，这里是指，只有第一层输入是input_dim，只有最后一层输出是output_dim
        if BN:
