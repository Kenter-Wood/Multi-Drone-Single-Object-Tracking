...
from models.multitemp.utils import recover_tokens, combine_tokens, get_distribution_target
...
class AbaViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.temp_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        torch.nn.init.normal_(self.temp_token, mean=0.0, std=0.02)
        ...
        self.pos_embed_cls = nn.Parameter(torch.zeros(1, 1, 192))
    ...
    def forward_features_act_token(self, z1, z2, z3, x, t_mask=None, s_mask=None, temp_token=None):
        # 输入搜索区域的尺寸
        ...
        # 如果当前没有传入temp_token，说明处理的是第一帧，用这里的初始temp_token初始化
        # 此时会对其进行梯度更新
        if temp_token is None:
            temp_token = self.temp_token.repeat(B, 1, 1)
        # 如果从外部传入了token，那么就不加入初始值，而是
        else:
            temp_token = temp_token.expand(B, -1, -1)
        ...
        z1 = z1 + self.pos_embed_z
        z2 = z2 + self.pos_embed_z
        z3 = z3 + self.pos_embed_z
        ...
        if self.dist_token is None:
            x = torch.cat((temp_token, cls_token, x), dim=1)
        else:
            x = torch.cat((temp_token, cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)
    ...
    def forward(self, z1, z2, z3, x, t_mask=None, s_mask=None, temp_token=None, temp_token_len=1):
        if self.args.act_mode == 4:
            x, aux_dict = self.forward_features_act_token(z1, z2, z3,x, t_mask=t_mask, s_mask=s_mask, temp_token=temp_token)
        else:
