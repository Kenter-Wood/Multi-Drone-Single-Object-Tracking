...
class BaseBackbone(nn.Module):
    ...
    def forward_features(self, z, x, temp_token=None, token_add_mode="add"):
        ...
        if self.add_cls_token:
            if temp_token:
                if token_add_mode == "add":
                    mix_token = temp_token + self.cls_token
                    mix_token = mix_token.expand(B, -1, -1)
                elif token_add_mode == "concat":
                    cls_tokens = self.cls_token.expand(B, -1, -1)
                    mix_token = torch.cat([cls_tokens, temp_token], dim=1)
                else:
                    raise ValueError(f"{token_add_mode} mode undefined, choose from 'add' and 'concat'")
            else:
                mix_token = self.cls_token.expand(B, -1, -1)
            mix_token = mix_token + self.cls_pos_embed
    ...
    def forward(self, z, x, temp_token, **kwargs):
        """
        ...
        Returns:
            ...
            attn : None
        """
        x, aux_dict = self.forward_features(z, x, temp_token)
