...
from models.mytrack.utils import combine_tokens, recover_tokens
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed_temp = nn.Parameter(torch.randn(1, 1, embed_dim))
    ...
    def forward_features(self, z, x, is_distill, temp_token, is_training):
        # 传入模板和搜索域，进行分块嵌入和位置嵌入
        # print(1, x.shape)
        x = self.patch_embed(x)
        z = self.patch_embed(z)
        # print(2, x.shape)

        temp_token = temp_token + self.pos_embed_temp
        if is_training:
            batch_size = x.shape[0]    
            # print(batch_size)                    
            temp_token_expand = temp_token.expand(batch_size, -1, -1)
        else:
            temp_token_expand = temp_token

        # print("dfnidnfidf", z.shape, self.pos_embed_z.shape)
        z += self.pos_embed_z
        x += self.pos_embed_x
        # 拼成一个序列
        # print(x.shape, z.shape, temp_token_expand.shape,"87878688")
        x = combine_tokens(z, x)
        x = combine_tokens(temp_token_expand, x)
        # 可能随机dropout
        x = self.pos_drop(x)
        # 激活概率
        probs_active = []
        # 激活阈值，前若干层强制激活，时序token更新幅度
        prob_thres = 0.5
        force_active = 2
        temp_update = 0.1
        ...
        for i, blk in enumerate(self.blocks):
            # 前两层固定激活，i代表层数，blk代表具体的网络层
            if i >= force_active and not is_distill:
                # 激活概率的计算：取序列(B, N, C)中通道的第一维(B,N,1), 把N投到一维得到分数
                prob_active = (blk.active_score_module(x[:, 1:, 0].clone())).sigmoid_()
                idx, _ = torch.where(prob_active > prob_thres)
                if len(idx) > 0:
                    if self.training:
                        x[idx] = blk(x[idx])
                ...
                x = blk(x)
        # 让这个时序token在模型中自行迭代，然后作为新的时序token
        # x[:, :1] = x[:, :1] + temp_update * torch.mean(x, dim=1, keepdim=True)
        lens_z = self.pos_embed_z.shape[1]
        lens_x = self.pos_embed_x.shape[1]
        x = recover_tokens(x, lens_z, lens_x)
        
        # print(temp_token_expand[0, 0, 0])
    ...
    def forward(self, z, x, temp_token,
                tnc_keep_rate=None,
                return_last_attn=False,
                is_distill=False,
                is_training=False):

        x, aux_dict = self.forward_features(z=z, x=x, is_distill=is_distill,
                                             temp_token=temp_token, is_training=is_training)
