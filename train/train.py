...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_port %d train/run_training.py " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
