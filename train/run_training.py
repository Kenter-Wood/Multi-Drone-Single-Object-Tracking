...
import settings as ws_settings
import warnings
...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    args:
        script_name: Name of experiment script in the "scripts/" folder.
    ...
    settings.cfg_file = os.path.join(prj_dir, 'scripts/%s/%s.yaml' % (script_name, config_name))
    ...
    if distill:
        ...
        expr_module = importlib.import_module('train_script_distill')
    else:
        expr_module = importlib.import_module('train_script')
